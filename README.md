...
[ironwallet.io/ai](https://ironwallet.io/ai) (GitHub Releases on the public
installer repo, when that channel is live). **Source is not published** — only
those binaries.
...
`npx -y @ironwallet/mcp-server` (fetched when the agent starts the server).
Claude, Codex, and Grok install the marketplace plugin from
...
4. Asks selected GUI apps to quit, then writes configs. One agent failure does
