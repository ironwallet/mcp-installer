...
|-------------|----------|---------------------|
| Claude Code | `npm install -g @anthropic-ai/claude-code` if `claude` is missing; `claude plugin marketplace add ironwallet/ironwallet-agent-kit`; `claude plugin install ironwallet-mcp@ironwallet --scope user` (retry without `--scope` if the flag is unknown) | Marketplace/plugin under `~/.claude` |
...
- **winget / Homebrew / apt / dnf** — Node.js; Git only if Claude is selected
  and `git` is missing.
- **npm** — `npm install -g @anthropic-ai/claude-code` only if Claude is
  selected and `claude` is not on PATH.
...
The next network hop is **not this process**: when an agent starts the MCP
entry, `npx -y @ironwallet/mcp-server` may download from the npm registry.
What that server sends after it is running is out of scope for this file.
